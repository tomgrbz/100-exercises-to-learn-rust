...
pub async fn echoes(first: TcpListener, second: TcpListener) -> Result<(), anyhow::Error> {
    let proc1 = tokio::spawn(echo(first));
    let proc2 = tokio::spawn(echo(second));
    let (out1, out2) = tokio::join!(proc1, proc2);
    out1??;
    out2??;
    Ok(())
}
async fn echo(listener: TcpListener) -> Result<(), anyhow::Error> {
    loop {
        ...
        tokio::spawn(async move {
            let (mut reader, mut writer) = conn.0.split();
            let _ = tokio::io::copy(&mut reader, &mut writer).await.unwrap();
        });
